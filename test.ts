...
export const describe = global.describe = function describe(groupName, fn) {
  let beforeEachFn = () => {};
  let afterEachFn = () => {};
...
export const spyOn = global.spyOn = function spyOn(object, methodName) {
  return {
    and: {
      returnValue(retVal) {
        spies.push({ object, methodName, fn: object[methodName] });
        object[methodName] = () => retVal;
      }
    }
  };
};
...
function removeSpies() {
  for (const { object, methodName, fn } of spies.splice(0)) {
